...
`<JWPlayer>` is a React Component that creates an instance of JW Player's web player. It allows the use of any event hooks that can be used on the standard player (as props), and provides access to player's API via a `componentDidMount` callback.
